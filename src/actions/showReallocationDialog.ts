...
export function showReallocationDialog(
    ...
    topSuggestions.forEach((suggestion, index) => {
      ...
      button.addEventListener("keydown", (e) => {
        if (e.key === "Enter" || e.key === " ") {
          ...
          e.preventDefault();
          const nextButton = buttonsContainer.querySelector(
            `button.suggestion-button:nth-child(${index + 2})`,
          ) as HTMLButtonElement;
          if (nextButton) {
          ...
          const prevButton = buttonsContainer.querySelector(
            `button.suggestion-button:nth-child(${index})`,
          ) as HTMLButtonElement;
          if (prevButton) {
            button.setAttribute("tabindex", "-1");
  ...
  emptyOption.value = "";
  emptyOption.textContent = isEventReallocation
    ? "Select an ambassador"
    : "Select or leave blank to unassign";
  ...
  const reallocatingEventName =
    isEventReallocation &&
    suggestions.length > 0 &&
    suggestions[0].items.length > 0
      ...
      if (ea) {
        ...
        if (eventDetails) {
          ...
          if (reallocatingEvent?.geometry?.coordinates) {
            const [reallocLon, reallocLat] =
              reallocatingEvent.geometry.coordinates;
        ...
        if (!reallocCoords && prospects) {
          const prospect = prospects
            .getAll()
            .find((p) => p.prospectEvent === reallocatingEventName);
        ...
        if (
          reallocCoords &&
          (ea.events.length > 0 ||
            (ea.prospectiveEvents && ea.prospectiveEvents.length > 0))
        ) {
  ...
  dropdown.addEventListener("keydown", (e) => {
    if (e.key === "ArrowUp" && dropdown.selectedIndex === 0) {
      e.preventDefault();
      const lastButton = content.querySelector(
        "button.suggestion-button:last-of-type",
      ) as HTMLButtonElement;
      if (lastButton) {
