...
5. **Last ambassador visit:** max finish date across **all** EAs and REAs with imported history; show all names tied on that date; **Not imported** when no visit history exists in Ambassy; **No visit on record** when history exists but no qualifying finish for that event. Display e.g. `Alex Sample; Kim Example — 13 Jun 2026` on Event Teams.
